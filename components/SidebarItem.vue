...
<template>
    <li v-if="item.subItems && item.subItems.length !== 0" :id="item.id">
        <template v-if="props.sidebarExpanded">
            <ULink :class="`rounded-md flex px-[6px] py-[10px] relative font-[5000] text-[15px] w-full items-center ${isActiveSubmenu()
                ...
                <UIcon :name="`${item.isMenuCollapsed ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'}`" class="ml-auto mr-2" />
                <span v-if="isActiveSubmenu()" class="w-[3px] bg-primary dark:bg-[#93A6FF] rounded-sm h-7 absolute right-[2px]" />
            </ULink>
            <ul class="mt-2">
                <li v-if="item.isMenuCollapsed" v-for="subItem in item.subItems" :key="subItem.id" class="pl-10">
                    <ULink :to="subItem.to" active-class="text-primary bg-white dark:bg-gray-500 dark:text-white"
                    ...
                    <div id="sidebarTasks" class="collapse"></div>
                </li>
            </ul>
        </template>
        <template v-else>
            <UPopover :popper="{ arrow: true, placement: 'right' }" class="overflow-visible">
                <ULink :class="`rounded-md flex px-[6px] py-[10px] relative font-[5000] text-[15px] w-full items-center ${isActiveSubmenu()
                    ? 'text-primary'
                    : 'text-gray-500 dark:text-gray-400 hover:text-primary dark:hover:text-gray-200'
                    }`" @click="updateMenu(item)">
                ...
                <template #panel>
                  <div class="p-4 text-sm text-white bg-gray-700">
                    <ul class="mt-2 text-center">
                        ...
                        <li v-for="subItem in item.subItems" :key="subItem.id" class="p-4 text-sm text-[#BFCEE0] bg-gray-700">
                            <ULink :to="subItem.to" >
                                {{ $t(subItem.label) }}
                            </ULink>
                        </li>
    ...
    <li v-else :id="item.id">
        <ULink v-if="props.sidebarExpanded" :to="item.to" active-class="text-primary bg-white dark:bg-gray-500 dark:text-white" v-slot="{ isActive }"
            ...
            <span>{{ $t(item.label) }}</span>
            <span v-if="isActive" class="w-[3px] bg-primary dark:bg-[#93A6FF] rounded-sm h-7 absolute right-[2px]" />
        </ULink>
        <UPopover v-else mode="hover" :popper="{ arrow: true, placement: 'right' }">
            <ULink :to="item.to" active-class="text-primary" v-slot="{ isActive }"
            inactive-class="text-gray-500 dark:text-gray-400 hover:text-primary dark:hover:text-gray-200"
            class="rounded-md flex px-[6px] py-[10px] relative font-[5000] text-[15px] w-full items-center"
            @click="updateMenu(item)">
